...
public class SourceCodeProvider {
    ...
    public static final Logger LOGGER = LoggerFactory.getLogger(SourceCodeProvider.class);
    ...
    public void setIncludeRoots(List<String> includeRoots, String baseDir) {
        for (String tmp : includeRoots) {
            ...
            try {
                ...
                LOGGER.error("cannot get canonical form of: '{}'", includeRoot);
            }

            if (includeRoot.isDirectory()) {
                LOGGER.debug("storing include root: '{}'", includeRoot);
                this.includeRoots.add(includeRoot);
            } else {
                LOGGER.warn("the include root '{}' doesn't exist", includeRoot.getAbsolutePath());
            }
        }
    }
    ...
    public File getSourceCodeFile(String filename, String cwd, boolean quoted) {
        ...
        if (result != null) {
            try {
                ...
                LOGGER.error("cannot get canonical form of: '{}'", result);
            }
        }
    ...
    public String getSourceCode(File file) {
        ...
        if (file.isFile()) {
            try {
                ...
                LOGGER.error("Cannot read contents of the file '{}'", file);
            }
        }
